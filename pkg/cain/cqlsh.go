...
func BackupKeyspaceSchema(iK8sClient, iDstClient interface{}, namespace, pod, container, keyspace, dstPrefix, dstPath, cassandraUsername, cassandraPassword string) (string, error) {
	clusterName, err := GetClusterName(iK8sClient, namespace, pod, container)
	if err != nil {
		return "", err
	}

	schema, sum, err := DescribeKeyspaceSchema(iK8sClient, namespace, pod, container, keyspace, cassandraUsername, cassandraPassword)
	if err != nil {
		return "", err
	}
...
func DescribeKeyspaceSchema(iK8sClient interface{}, namespace, pod, container, keyspace, cassandraUsername, cassandraPassword string) ([]byte, string, error) {
	command := []string{fmt.Sprintf("DESC %s;", keyspace)}
	schema, err := Cqlsh(iK8sClient, namespace, pod, container, command, cassandraUsername, cassandraPassword)
	if err != nil {
...
func RestoreKeyspaceSchema(srcClient, iK8sClient interface{}, srcPrefix, srcPath, namespace, pod, container, keyspace, schema string, parallel int, bufferSize float64, cassandraUsername, cassandraPassword string) (string, error) {
	...
	if _, err := CqlshF(iK8sClient, namespace, pod, container, schemaTmpFile, cassandraUsername, cassandraPassword); err != nil {
		return "", err
	}
	_, sum, err := DescribeKeyspaceSchema(iK8sClient, namespace, pod, container, keyspace, cassandraUsername, cassandraPassword)

	return sum, err
}

// TruncateTables truncates the provided tables in all pods
func TruncateTables(iK8sClient interface{}, namespace, container, keyspace string, pods, tables, materializedViews []string, cassandraUsername, cassandraPassword string) {
	bwgSize := len(pods)
	bwg := utils.NewBoundedWaitGroup(bwgSize)
	for _, pod := range pods {
		bwg.Add(1)

		go func(iK8sClient interface{}, namespace, container, keyspace, pod, cassandraUsername, cassandraPassword string) {
			for _, table := range tables {
				...
				_, err := Cqlsh(iK8sClient, namespace, pod, container, command, cassandraUsername, cassandraPassword)
				if err != nil {
					log.Fatal(err)
				}
			}
			bwg.Done()
		}(iK8sClient, namespace, container, keyspace, pod, cassandraUsername, cassandraPassword)
...
func GetMaterializedViews(iK8sClient interface{}, namespace, container, pod, keyspace, cassandraUsername, cassandraPassword string) ([]string, error) {

	command := []string{fmt.Sprintf("SELECT view_name FROM system_schema.views WHERE keyspace_name='%s';", keyspace)}
	output, err := Cqlsh(iK8sClient, namespace, pod, container, command, cassandraUsername, cassandraPassword)
	if err != nil {
		log.Fatal(err)
	}
...
func Cqlsh(iK8sClient interface{}, namespace, pod, container string, command []string, cassandraUsername, cassandraPassword string) ([]byte, error) {
	k8sClient := iK8sClient.(*skbn.K8sClient)

	if cassandraPassword != "" {
		command = append([]string{"cqlsh", "-u", cassandraUsername, "-p", cassandraPassword, "-e"}, command...)
	} else {
...
func CqlshF(iK8sClient interface{}, namespace, pod, container string, file string, cassandraUsername, cassandraPassword string) ([]byte, error) {
	k8sClient := iK8sClient.(*skbn.K8sClient)

	command := []string{}
	if cassandraPassword != "" {
		command = []string{"cqlsh", "-u", cassandraUsername, "-p", cassandraPassword, "-f", file}
	} else {
